...
label prompt_menu:
    ...
    python:
        ...
        if len(unseen_events)>0:
            talk_menu.append(("{b}Не показанное.{/b}", "unseen"))
        talk_menu.append(("Задать вопрос.", "prompt"))
        if len(repeatable_events)>0:
            talk_menu.append(("Повторить разговор.", "repeat"))
        talk_menu.append(("До свидания.", "goodbye"))
        talk_menu.append(("Неважно.","nevermind"))

        renpy.say(m, "О чем бы ты хотел поговорить?", interact=False)
