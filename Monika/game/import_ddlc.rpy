...
label import_ddlc_persistent:
    ...
    if save_path:
        ...
        "Сохраненные данные Doki Doki Literature Club были найдены в [save_path]."
        menu:
            "Хотите импортировать Doki Doki Literature Club сохранить данные в [config.name]?\n(DDLC не будет затронуто)"
            "Да, импортировать данные DDLC.":
                pause 0.3
                pass
        ...
        $quick_menu = False
        "Сохраненные данные Doki Doki Literature Club не были найдены."
        menu:
            "Сохранить данные не будут импортированы."
            "Ок":
                pause 0.3
                pass
        return
    ...
    if persistent.first_run:
        ...
        menu:
            "Предыдущие сохранения Monika After Story были найдены.\nЗаменить или объединить с DDLC?"
            "Объединить сохраненные данные.":
                $merge_previous=True #Time to merge data

            "Удалить данные After Story.":
                menu:
                    "Monika After Story данные будут удалены. Это нельзя отменить. Вы уверены?"
                    "Да":
                        m "Ты действительно не изменился."
                    "Нет":
                        jump save_merge_or_replace
            "Назад.":
                "DDLC данные можно импортировать позже в меню настроек."
                return
...
label merge_unmatched_names:
    menu:
        "Имена файлов не совпадают. Что бы вы хотели оставить?"
