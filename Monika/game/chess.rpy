...
default persistent._mas_chess_stats = {"Побед": 0, "Порожений": 0, "Ничья": 0}
define mas_chess.CHESS_SAVE_PATH = "/chess_games/"
define mas_chess.CHESS_SAVE_EXT = ".pgn"
define mas_chess.CHESS_SAVE_NAME = "abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ ЁЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮйцукенгшщзхъфывапролджэячсмитьбю-_0123456789"
define mas_chess.CHESS_PROMPT_FORMAT = "{0} | {1} | Ход: {2} | Ты: {3}"
...
init 1 python in mas_chess:
    ...
    CHESS_MENU_NEW_GAME_VALUE = "Новая игра"
    ...
    def isInProgressGame(filename, mth):
        ...
        if pgn_game.headers["Результат"] != "*":
            return None

        # now which one is the player?
        if pgn_game.headers["Белый"] == mth:
            the_player = "Черный"
        elif pgn_game.headers["Черный"] == mth:
            the_player = "Белый"
        ...
        return (
            CHESS_PROMPT_FORMAT.format(
                pgn_game.headers["Дата"].replace(".","-"),
                pgn_game.headers["Событие"],
        ...
        if is_platform_good_for_chess():
            ...
            try: 
                ...
                raise ChessException(
                    "Не удалось создать папку игры в шахматы '{0}'".format(
                        file_path
                    )
                )
        ...
        class ChessDisplayable(renpy.Displayable):
            ...
            def __init__(self, player_color, pgn_game=None):
                ...
                if not is_platform_good_for_chess():
                    ...
                    raise ArchitectureError('Ваша Операционная система не поддерживает шахматную игру.')
...
label game_chess:
    ...
    m 1b "Хочешь поиграть в шахматы? Хорошо~"
...
label demo_minigame_chess:
    ...
    if pgn_files:
        ...
        if len(pgn_games) > 0:
            ...
            m 1a "У нас еще есть [game_s_dialog]."
            show monika at t21
            $ renpy.say(m, "Выбери игру в которую хочешь играть.", interact=False)
    ...
    menu:
        ...
        "Выбери сама":
            ...
            if choice:
                $ player_color = ChessDisplayable.COLOR_WHITE
                m 2a "Смотри, я играю белыми! Давай начнем!"
            else:
                $ player_color = ChessDisplayable.COLOR_BLACK
                m 2a "Смотри, я играю черными! Давай начнем!"
...
label mas_chess_game_start:
    ...
    if game_result == "*":
        ...
        if is_surrender and num_turns <= 4:
            m 1e "Давай, не сдавайся так легко."
        else:
            m 1b "Я выиграла!"

        if persistent.chess_strength>0:
            m 1j "В следующий раз я не поддамся."
            $persistent.chess_strength += -1
        else:
            m 1l "В следующий раз я буду помягче с тобой"
        ...
        m 2a "Ты победил! Поздравляю."
        if persistent.chess_strength<20:
            m 2 "В следующий раз я обязательно выйграю!"
            $persistent.chess_strength += 1
        else:
            m 2b "Ты действительно удивительный игрок!"
            m 3l "Ты ведь не жульничаешь?"
        ...
        menu:
            ...
            "Да":
                label mas_chess_savegame:
                    ...
                    m 1j "Я сохранила нашу игру в '[display_file_path]'!"

                    if not renpy.seen_label("mas_chess_pgn_explain"):

                        label mas_chess_pgn_explain:
                            m 1a "Это в формате под названием Portable Game Notation."
                            m "Этот файл можно открыть в программе просмотра PGN."

                            if game_result == "*": # ongoing game
                                m 1n "Можно отредактировать этот файл и изменить исход игры,{w} но я уверена, что ты не сделаешь этого."
                                m 1e "Так ведь, [player]?"
                                menu:
                                    "Конечно":
                                        m 1j "Ура~"
...
label mas_chess_end:
    if is_monika_winner:
        ...
        m 1a "Вполне нормально ошибаться во время игры."
        m 1j "Помни, что важно уметь учиться на своих ошибках."
    elif game_result == "*":
        # TODO: this really should be better
        m 1a "Хорошо, [player], давай продолжим эту игру."
    else:
        m 2b "Удивительно, сколькому еще я должена научиться."
        m 2a "Я действительно не против проигрыша, пока я могу чему-то научиться."
        m 1j "В конце концов, я играю с тобой."
