    ...
    python:
        ...
        class PongDisplayable(renpy.Displayable):
            ...
            def __init__(self):
                ...
                self.ctb = Text(_("Нажми что бы начать"), size=36)
...
label game_pong:
    ...
    m 1a "Хочешь поиграть в Pong? Окей!"
...
label mas_pong_dlg_winner_end:
    m 4e "Я не могу получать удовольствия от такой простой игры..."
    m 1a "По крайней мере, мы можем играть в нее вместе."
    m "A-ха-ха!"
    m 1b "Спасибо, что позволили мне победить, [player]."
    m 1a "Только старшеклассники серьезно проигрывают в Pong, верно?"
    m "Э-хе-хе~"   
    return
...
label mas_pong_dlg_loser_end:
    m 1d "Вау, я старалась."
    m 1a "Ты, должно быть, тренировался, чтобы выйграть."
    m "Это то, чем ты можешь гордиться?"
    m 1j "Я думаю, ты хотел произвести на меня впечатление, [player]~"   
    return

# quick dialogue shown when monika is the pong loser
label mas_pong_dlg_loser_fast:
    m 1e "Я обыграю тебя в следующий раз, [player]."
    return
