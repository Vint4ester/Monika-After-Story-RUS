...
#    $ quit_label = Text("Press 'Z' to quit", size=36)
    ...
    m 1j "Хочешь поиграть на пианино?"
...
label mas_piano_songchoice:
    ...
    if len(song_list) > 1:
        ...
        menu:
            m "Играть будешь самостоятельно или нет, [player]?"
            "Нет":
                m "Какую песню?"
                ...
                if pnml != "None":
                    ...
                    m 1j "Я так рада слышать, как ты играешь, [player]!"
    ...
    m 1a "Тогда сыграй для меня, [player]~"
...
label mas_piano_result_none:
    ...
    m 1e "Мне очень нравится как ты играешь."
    m 1j "Обещаешь сыграть для меня в следующи раз?"
    return
...
label mas_piano_yr_win:
    m 1m "Очень хорошо, [player]."
    m "Но..."
    m 1n "Ты сможешь лучше если попрактикуешься..."
    m 1l "Э-хе-хе~"
    return

# shown if player FCs
label mas_piano_yr_fc:
    m 1b "Это было чудесно, [player]!"
    m 1j "Я не знал что ты так хорошо играешь на пианино."
    m 1a "Может, как-нибудь поиграем вместе!"
    return

# shown if player did not complete song and had more fails than passes
label mas_piano_yr_fail:
    m 1o "..."
    m 1e "Ничего страшного, [player]."
    m 1j "По крайней мере ты пытался."
    return

# shown if player did not complete song but had more passes than fails
label mas_piano_yr_prac:
    m 1a "Это было действительно круто, [player]!"
    m 3b "Если попрактикуешься, сможешь играть мою песню прекрасно."
    m 1j "Не забывай практиковаться каждый день для меня, хорошо?"
    return
...
#        CONFIG_CHANGE = "monika 3a"
        ...
        def __init__(self, mode, pnml=None):
            ...
            self._config_wait_help = Text(
                "Нажми на розовую область, что бы изменить раскладку клавиш на пианино",
                font=gui.default_font,
                size=gui.text_size,
                color="#fff",
                outlines=[]
            ...
            self._config_change_help = Text(
                "Нажми на клавишу которую хочешь установить для клавиши пианино ",
                font=gui.default_font,
                size=gui.text_size,
                color="#fff",
                outlines=[]
