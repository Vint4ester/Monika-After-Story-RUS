...
# hangman stuff only
define hm_ltrs_only = "абвгдеёжзийклмнопрстуфхцчшщыъьэюя?!-"
...
label game_hangman:
    ...
    m 2b "Ты хочешь поиграть в Виселицу? Хорошо!"
    # setup positions
    ...
    while not done:
        ...
        if chances == 0:
            ...
            if guess == "?": # hint text
                ...
                m 1n "[player]..."
                m "Ты должен хотя бы лоиграть до конца..."
                m 1f "Отказ является признаком плохой решимости."
                if chances > 1:
                    m "Я имею в виду, тебе придется пропустить [chances] больше букв что бы проиграть."
                else:
                    m "Я имею в виду, тебе придется пропустить [chances] больше букв что бы проиграть."
                m 1e "В следующий раз ты будешь играть до конца, [player]? Ради меня?"
            else:
                python:
                    if guess in word:
...
label hangman_game_end:
    ...
    m 1d "Виселица на самом деле, довольно сложная игра"
    m "Тебе нужно иметь хорошый словарный запас, что бы ты мог угадать слово."
    m 1j "Лучший способ пополнить его, это читать больше книг!"
    m 1a "Я буду очень счастлива если ты сделаешь это для меня, [player]."
    return
